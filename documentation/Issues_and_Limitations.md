...
- ✏️ **Check Typos:**
  Check for typos especially when implementing manual informatin such as the type of schedulers (WCA`, `WCD`, `NCA`, and `NCD`), parameter values, etc.
-  🧭 **Consult the Logs:**
  Look for `!!!` markers in the MATLAB console. These error meesages are generated by TAS-Forge to singal critical errors. Refer to this document for an understanding of their likely root causes.
