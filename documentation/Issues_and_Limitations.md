...
### 💢 Scheduler Selection:
TAS-Forge supports only four schedulers: **WCA**, **WCD**, **NCA**, and **NCD**. If any other scheduler is selected, or if there is a typo in entering the scheduler name during the GCL generation via the `generate_GCL_output` script, the following error will be triggered:
...
- ✅ **Environment Versions:**
  Ensure you have the correct versions of **MATLAB**, **CPLEX**, **OMNeT++** and **INET** installed. Version mismatches can cause unexpected behavior.
- 🪜 **Workflow Order:**
  Follow the recommended execution sequence outlined in the [User Guide](User_Guide.md). Running scripts out of order will result in configuration errors.
- 📝 **MATLAB Workspace:**
  Ensure that the correct variables are loaded into the MATLAB workspace. Use the command `whos` in MATLAB to inspect active variables.
- 📚 **Files Generation:**
  Verify that the correct files are loaded including the `.csv`,`.txt`,`.ned`, and `.ini` files are correctly generated and can be executed without errors.
- ✏️ **Check Typos:**
  Check for typos especially when implementing manual informatin such as the type of schedulers (`WCA`, `WCD`, `NCA`, and `NCD`), parameter values (`T_sync`,`T_period`), etc.
-  🧭 **Consult the Logs:**
  Look for `!!!` markers in the MATLAB console. These error meesages are generated by TAS-Forge to singal critical errors. Refer to this documentation for an understanding of their likely root causes.
- 🔄 **Clear and Reload Workspace:**
  If all else fails, use commands like `clear all`, `clc`, and `close all` to reset the MATLAB workspace. Consider deleting previously *generated* files and restarting the workflow from the beginning for a clean run.
