...
outputFile = fopen('CPLEX_Code_Output/output_CPLEX_code_NCD.mod', 'w');
            ...
            for l = j+1:length(routes_included)
                ...
                for k = 1:k_max
                    ...
                    constraint = sprintf('abs((OFF_%d_%s + (alpha - 1)*T_period[%d]) - (OFF_%d_%s + (beta - 1)*T_period[%d])) <= %d*(hp - 1);', ...
                        streamID1, switchID, streamID1, streamID2, switchID, streamID2, k);
...
for i = 1:length(routes)
    ...
    for j = 1:length(switches)
        if eq(j,1)
            ft_constraint = sprintf('OFF_%d_%s >= OFF_%d_%s + ceil(L[%d] + maxl(abs(p_%s - p_%s)*T_sync, abs(p_%s - p_GM1)*T_sync, abs(p_%s - p_GM1)*T_sync));',i,switches{j},i,sources{1},i,...
                switches{j},sources{1},switches{j}, sources{1});
        else
            ft_constraint = sprintf('OFF_%d_%s >= OFF_%d_%s + ceil(L[%d] + maxl(abs(p_%s - p_%s)*T_sync, abs(p_%s - p_GM1)*T_sync, abs(p_%s - p_GM1)*T_sync));',i,switches{j},i,switches{j-1},i,...
                switches{j},switches{j-1},switches{j},switches{j-1});
        end
    % Write the constraint to the file
    fprintf(outputFile, '\n%s', ft_constraint);
    end
        ...
        for j = 1:length(streamList)
            for k = j+1:length(streamList)
                ...
                constraint1 = sprintf(['OFF_%d_%s + alpha*T_period[%d] <= OFF_%d_%s + beta*T_period[%d] +' ...
                    '(L[%d] - maxl(0, (p_%s - p_%s)*T_sync, (p_%s - p_GM1)*T_sync, (p_GM1 - p_%s)*T_sync))||\n'],...
...
for i = 1:length(routes)
    ...
    e2e_constraint_1 = sprintf('lambda_%d == OFF_%d_%s  - OFF_%d_%s + ceil(trans_delay[%d] + prop_delay);',i,i,switches{length(switches)},i,sources{1},i);
...
fprintf(outputFile, 'var solFile = new IloOplOutputFile("output_CPLEX_solution_NCD.txt");\n\n');
...
disp('Output file "output_CPLEX_code_generator_NCD.mod" has been generated in the CPLEX_Code_Output folder.');
