...
outputFile = fopen('CPLEX_Code_Output/output_CPLEX_code_NCA.mod', 'w');
...
outputLine = sprintf('\nint hp = %d;', hp); % Print the hyperperiod
fprintf(outputFile, '%s', outputLine);
...
fprintf(outputFile, '\n\n%s', '// The clock drift values of devices in the network');
devices_names   = nodeData.("Node_Name"); % Extrat the device list
clock_drift     = nodeData.('Clock_Drift'); % Extract the clock drift values 

for i = 1:length(clock_drift)
    if ne(startsWith(devices_names(i), 'sink'),1) % Do not show that sink clock drift
        outputLine = sprintf('\nfloat p_%s = %f;', string(devices_names(i)),clock_drift(i)); % Print the clock drift values
        fprintf(outputFile, '%s', outputLine);
    end
end

T_sync = networkData.('Sync Periodicity');
fprintf(outputFile, '\n\n%s', '// The synchronization periodicity');
outputLine = sprintf('\nfloat T_sync = %f;', T_sync);
fprintf(outputFile, '%s', outputLine);
...
for i = 1:length(routes)
    ...
    for j = 1:length(switches)
        if eq(j,1)
            ft_constraint = sprintf(['OFF_%d_%s >= OFF_%d_%s + floor(L[%d] ' ...
                '+ minl(0, (p_%s - p_%s)*T_sync, (p_GM1 - p_%s)*T_sync, (p_%s - p_GM1)*T_sync));'],...
                i,switches{j},i,sources{1},i,...
                switches{j},sources{1},sources{1},switches{j});
        else
            ft_constraint = sprintf(['OFF_%d_%s >= OFF_%d_%s + floor(L[%d]'...
                '+ minl(0, (p_%s - p_%s)*T_sync, (p_GM1 - p_%s)*T_sync, (p_%s - p_GM1)*T_sync)'...
                ' - minl(0, (p_%s - p_%s)*T_sync, (p_GM1 - p_%s)*T_sync, (p_%s - p_GM1)*T_sync));'],...
                i,switches{j},i,switches{j-1},i,...
                switches{j},sources{1},sources{1},switches{j},...
                switches{j-1},sources{1},sources{1},switches{j-1});
        end
    % Write the constraint to the file
    fprintf(outputFile, '\n%s', ft_constraint);
    end
...
for i = 1:length(topologyData)
    ...
    if startsWith(topologyData(i,1),'switch') && gt(length(routes_included),1)
        ...
        % Specify the switch and egress port
        for j = 1:length(routes_included)
            for k = j+1:length(routes_included)
                streamID1 = routes_included(j);
                streamID2 = routes_included(k);
                
                % Extract the sources from the Stream IDs
                var1 = streamData.("Route")(streamID1);
                var2 = streamData.("Route")(streamID2);
                var1_str_split = strsplit(var1{1},',');
                var2_str_split = strsplit(var2{1},',');
                sourceID1 = string(var1_str_split(1));
                sourceID2 = string(var2_str_split(1));
                
                % Write the loop structure for repetitions
                fprintf(outputFile, 'forall(alpha in 0..repetitions[%d]-1, beta in 0..repetitions[%d]-1) {\n', streamID1, streamID2);
                % Write the first condition
                constraint1 = sprintf(['OFF_%d_%s - OFF_%d_%s <= (alpha*T_period[%d] - beta*T_period[%d])'...
                    '- ceil(trans_delay_var[%d] - minl(0, (p_%s - p_%s)*T_sync, (p_%s - p_GM1)*T_sync, (p_GM1 - p_%s)*T_sync)'...
                    ' + maxl(0, (p_%s - p_%s)*T_sync, (p_%s - p_GM1)*T_sync, (p_GM1 -p_%s)*T_sync) + 2 ) ||\n'], ...
                    streamID2, switchID, streamID1, switchID, streamID1, streamID2, ...
                    streamID2, switchID, sourceID2, switchID, sourceID2,...
                    switchID, sourceID2, switchID, sourceID2);
                fprintf(outputFile, '%s', constraint1);
            
                % Write the second condition
                constraint2 = sprintf(['OFF_%d_%s - OFF_%d_%s <= (beta*T_period[%d] - alpha*T_period[%d])'...
                    '- ceil(trans_delay_var[%d] - minl(0, (p_%s - p_%s)*T_sync, (p_%s - p_GM1)*T_sync, (p_GM1 - p_%s)*T_sync)'...
                    ' + maxl(0, (p_%s - p_%s)*T_sync, (p_%s - p_GM1)*T_sync, (p_GM1 -p_%s)*T_sync) + 2 ); \n'], ...
                    streamID1, switchID, streamID2, switchID, streamID2, streamID1, ...
                    streamID1, switchID, sourceID1, switchID, sourceID1,...
                    switchID, sourceID1, switchID, sourceID1);
...
for i = 1:length(topologyData)
    % Gather the list of routes
    streamList  = str2double(strsplit(topologyData(i,5)));
    deviceID    = topologyData{i, 1}; %Load the current switch in question

    %Only proceed if the switch has more than 1 stream
    if gt(length(streamList),1) && startsWith(deviceID,'switch') 
        fprintf(outputFile, '\n// Port of %s towards %s \n', deviceID, topologyData(i,3));
        ...
        for j = 1:length(streamList)
            for k = j+1:length(streamList)
                ...
                % Write the first condition
                constraint1 = sprintf(['OFF_%d_%s + alpha*T_period[%d] <= OFF_%d_%s + beta*T_period[%d] +' ...
                    'ceil(L[%d] - maxl(0, (p_%s - p_%s)*T_sync, (p_%s - p_GM1)*T_sync, (p_GM1 - p_%s)*T_sync))||\n'],...
                    streamID1, switchID, streamID1, streamID2, prevDevice2, streamID2,...
                    streamID2,switchID,prevDevice2,switchID,prevDevice2);
                fprintf(outputFile, '%s', constraint1);
                
                % Write the second condition
                constraint2 = sprintf(['OFF_%d_%s + beta*T_period[%d] <= OFF_%d_%s + alpha*T_period[%d] +'...
                    'ceil(L[%d] - maxl(0, (p_%s - p_%s)*T_sync, (p_%s - p_GM1)*T_sync, (p_GM1 - p_%s)*T_sync));\n'],...
                    streamID2, switchID, streamID2, streamID1, prevDevice1, streamID1,...
                    streamID1,switchID,prevDevice1,switchID,prevDevice1);
                fprintf(outputFile, '%s', constraint2);
...
for i = 1:length(routes)
    ...
    e2e_constraint_1 = sprintf(['lambda_%d == OFF_%d_%s  - OFF_%d_%s + (trans_delay[%d] + prop_delay' ...
        '- minl(0,(p_%s - p_%s), (p_GM1 - p_%s), (p_%s - p_GM1))*T_sync);'],...
        i,i,switches{length(switches)},i,sources{1},i,...
        switches{length(switches)},sources{1},sources{1},switches{length(switches)});
...
fprintf(outputFile, 'var solFile = new IloOplOutputFile("output_CPLEX_solution_NCA.txt");\n\n');
...
disp('Output file "output_CPLEX_code_generator_NCA.mod" has been generated in the CPLEX_Code_Output folder.');
