...
outputFile = fopen('CPLEX_Code_Output/output_CPLEX_code_WCA.mod', 'w');
...
for i = 1:height(streamData)
    ...
    for j = 1:length(devices) - 1 %Source to last switch
        deviceName = strtrim(devices{j});  % Remove any leading/trailing spaces
        if eq(j,1)
            outputLine = sprintf('0 <= OFF_%d_%s <= hp;', streamID, deviceName);
        else
            outputLine = sprintf('0 <= OFF_%d_%s <= hp;', streamID, deviceName);
        end
        % Write the output line to the file
    ...
    if startsWith(topologyData(i,1),'switch') && gt(length(routes_included),1)
        ...
        for j = 1:length(routes_included)
            for k = j+1:length(routes_included)
                ...
                constraint1 = sprintf('OFF_%d_%s - OFF_%d_%s <= (alpha*T_period[%d] - beta*T_period[%d]) - ceil(trans_delay[%d] + 2*delta + 1) ||\n', ...
                ...
                constraint2 = sprintf('OFF_%d_%s - OFF_%d_%s <= (beta*T_period[%d] - alpha*T_period[%d]) - ceil(trans_delay[%d] + 2*delta + 1);\n', ...
...
for i = 1:length(routes)
    ...
    e2e_constraint_1 = sprintf('lambda_%d == OFF_%d_%s  - OFF_%d_%s + (trans_delay[%d] + prop_delay + delta);',i,i,switches{length(switches)},i,sources{1},i);
...
fprintf(outputFile, 'var solFile = new IloOplOutputFile("output_CPLEX_solution_WCA.txt");\n\n');
...
disp('Output file "output_CPLEX_code_generator_WCA.mod" has been generated in the CPLEX_Code_Output folder.');
